...
def build_parser(
    ...
    build_parser.add_argument("--hidden-layers", type=int, nargs='*',
                              default=[],
                              help="int*")
    build_parser.add_argument(
        "--activation",
        choices=['tanh', 'softmax', 'relu', 'softplus', 'sigmoid'],
        help="int*")
    build_parser.add_argument(
    ...
    def build(args):
        model = Sequential()
        total_nodes = [args.input_dim] + args.hidden_layers + [args.output_dim]
        activations = [args.activation for i in
                       range(len(total_nodes)-2)] + ['softmax']

        model.add(Dense(total_nodes[1], input_dim=total_nodes[0],
        ...
        for i in range(len(total_nodes[2:])):
            model.add(Dense(total_nodes[i], input_dim=args.input_dim,
