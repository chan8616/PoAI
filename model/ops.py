...
#
#     conv_base_name = 'conv_'+name
#     bn_base_name = 'bn_'+name
#     bn = [batch_norm(name=bn_base_name+'i{}'.format(i)) for i in range(3)]
#     x = _input
#     for i in range(3):
#         ks = ks if i == 1 else 1
#         padding = 'SAME' if i == i else 'VALID'
#         x = relu(bn[i](conv2d(x, filters[i], ks, 1, name=conv_base_name+'i{}'.format(i), biases=False, padding=padding)))
#     return relu(tf.add(_input+x))
#
# def conv_block(_input, ks, filters, name, st=(2,2)):
#     conv_base_name = 'conv_'+name
#     bn_base_name = 'bn_'+name
